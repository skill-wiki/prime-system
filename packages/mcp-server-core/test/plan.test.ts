/**
 * `prime_plan` and the MCP protocol layer.
 *
 ...
 *
 * §7.3 — *nothing* in this package's suite went through `createPrimeMcpServer`
...
import { createPrimeMcpServer } from "../src/index";
import { executePrimePlan, executePrimeQuery, type ServeOptions } from "../src/serve";
...
describe("executePrimePlan", () => {
  ...
  it("treats an empty seeds array as no retrieval signal", () => {
    expect("error" in executePrimePlan({ seeds: [] }, serve())).toBe(true);
  });

  it("returns the selection plan itself, including the arithmetic behind it", () => {
    const outcome = executePrimePlan({ query: "tea" }, serve());
  ...
  it("plans from seeds alone, so the graph axis is reachable without query text", () => {
    const outcome = executePrimePlan({ seeds: [TEA] }, serve());
    if ("error" in outcome) throw new Error(outcome.error);
    expect(outcome.plan.candidates.length).toBeGreaterThan(0);
  });

  it("is the same plan prime_query renders, not a second opinion", () => {
    ...
    const options = serve();
    const plan = executePrimePlan({ query: "tea" }, options);
    if ("error" in plan) throw new Error(plan.error);
    const query = executePrimeQuery({ scope: "atoms", query: "tea", limit: 10 }, options);
  ...
  it("reports a narrower plan when a required facet excludes everything", () => {
    const outcome = executePrimePlan({ query: "tea", kind: "no-such-type" }, serve());
    if ("error" in outcome) throw new Error(outcome.error);
...
describe("the MCP protocol surface", () => {
  async function connect() {
    const instance = createPrimeMcpServer({ primeDir: CORPUS, environment: {}, stderr: { error: () => {} } });
  ...
  it("advertises every tool with a non-empty inputSchema", async () => {
    ...
    expect(names).toEqual(["prime_plan", "prime_query", "prime_resource"]);
  ...
  it("requires `scope` on prime_query and `uri` on prime_resource at the protocol level", async () => {
    const { client } = await connect();
    const listed = await client.listTools();
    const required = new Map(listed.tools.map((tool) => [tool.name, tool.inputSchema.required ?? []]));
    expect(required.get("prime_query")).toContain("scope");
    expect(required.get("prime_resource")).toContain("uri");
    await client.close();
  });

  it("reports one and the same snapshot from query, plan and show (Phase 0 acceptance 1)", async () => {
    const { client } = await connect();
    const atoms = payload(await client.callTool({ name: "prime_query", arguments: { scope: "atoms", limit: 2 } }));
    const show = payload(await client.callTool({ name: "prime_query", arguments: { scope: "show", id: TEA } }));
    const plan = payload(await client.callTool({ name: "prime_plan", arguments: { query: "tea" } }));
    expect(plan.snapshot).toEqual(atoms.snapshot);
    expect(show.snapshot).toEqual(atoms.snapshot);
    await client.close();
  });

  it("delivers arguments to prime_plan rather than silently defaulting them", async () => {
    const { client } = await connect();
    const refused = await client.callTool({ name: "prime_plan", arguments: {} });
    expect((refused as { content: { text: string }[] }).content[0]!.text).toMatch(/requires `query` or `seeds`/);
    const planned = payload(await client.callTool({ name: "prime_plan", arguments: { query: "tea" } }));
  ...
  it("reports an out-of-schema argument as an error result instead of querying something else", async () => {
    const { client } = await connect();
    const result = await client.callTool({ name: "prime_query", arguments: { scope: "not-a-scope" } }) as {
      isError?: boolean;
      content: { text: string }[];
    };
