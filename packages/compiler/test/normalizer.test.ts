...
test("protocol _meta becomes provenance instead of a domain field",()=>{const p=parse('unit X : Ticket { title: "x" _meta: { license: "MIT" method: audited } }',"meta.prime");if(p.ast.type!=="UnitDeclaration")throw new Error("not unit");const n=normalizeUnit(p.ast,modelResult.value,context);expect(n.ok).toBe(true);if(n.ok){expect(n.value.fields._meta).toBeUndefined();expect(n.value.provenance.attributes).toEqual({license:"MIT",method:"audited"});}});
test("a relation declaration need not be repeated on every type",()=>{const compat=loadModel(join(import.meta.dir,"../../../compat/prime-v1-model"));if(!compat.ok)throw new Error("compat failed");const p=parse('rule Child { id: "@x/child" version: "1.0.0" extends: @x/base }');if(p.ast.type!=="AtomDeclaration")throw new Error("not atom");const n=normalizePrimeV1Atom(p.ast,compat.value,context);expect(n.ok).toBe(true);if(n.ok){expect(n.value.fields.extends).toBeUndefined();expect(n.value.relations.map(edge=>edge.relationRef)).toEqual(["extends"]);}});
