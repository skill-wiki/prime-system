...
Claude Code 重启后，工具会以 `mcp__skill-wiki__prime_query` 的名字
...
起成子进程，设置 `PRIME_DIR` 环境变量，用 `Content-Length` 帧化的 JSON-RPC
...
1. **MCP 流量受控。** 不管 query 命中多少 atom，`prime_query` 调用
...
4. **可以和别的工具组合。** Agent 可以把 `prime_query` 结果接进
...
- **不做多轴检索。** 给前端 brief 选「register、pattern、motion、
  ...
  `prime_query` 只按 query 字符串和边图返回；按轴排序是 wrapper 的事。
...
| `prime_query` | 形状和核心一样；wrapper 把 `scope` 扩成前端专属的 `mandate`、`checklist`、`gallery` 等。 |
...
System 仓库故意停在 `prime_query`。这一个工具加上 agent 的 `Read`，
